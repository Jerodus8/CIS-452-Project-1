 ...
 * ring_comm.c
 *
 * Circular communication ring with "apple" token synchronization.
 *
 * Usage:
 *   Compile: gcc -std=c11 -O2 -Wall -Wextra -o ring_comm ring_comm.c
 *   Run:     ./ring_comm
 *
 * Parent (node 0) will prompt for k, then spawn processes.
 * Parent seeds the ring with an empty message and starts the apple.
 *
 * Controls:
 *   - Enter messages (possibly multi-word) when parent prompts after it receives the apple.
 *   - Press Ctrl-C in the parent terminal to gracefully shutdown the simulation.
 *
